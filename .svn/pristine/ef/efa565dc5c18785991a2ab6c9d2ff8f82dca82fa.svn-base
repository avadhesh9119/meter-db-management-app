...
@Component
public class InstantDevicesInfoMdmCaster {
	...
	public void prepareInstantData(String outputList, List<InstantDevicesInfoResponse> ispResponseList, MeterDataVisualizationReq req) throws Exception {
		...
		for (DevicesInfo devInfo : instantData) {
			...
			if(devInfo.getDevice_type().equalsIgnoreCase(MdmExternalConstants.DeviceTypes.CT_METER)) {
								ispResponse.setRelayStatus("-");
			}else {
				ispResponse.setRelayStatus(MdmExternalConstants.Status.RELAY_STATUS.equalsIgnoreCase(devInfo.getRelay_status())
						? MdmExternalConstants.Status.CONNECTED
						: MdmExternalConstants.Status.DISCONNECTED);
			}
