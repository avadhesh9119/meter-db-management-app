...
@Component
public class DevicesCommandsBatchLogsCaster {
	private static final Logger LOG = LoggerFactory.getLogger(DevicesCommandsBatchLogsCaster.class);
	...
	public void prepareDevicesCommandBatchLogs(String outputList,
		...
		for (DevicesCommandsBatchLogs ispData : batchRequestList) {
			...
			if(ispData.getCommand_name().contains(":") && ispData.getCommand_name().contains("{") && ispData.getCommand_name().contains("}")) {
				ispResponse.setCommandType(Constants.CommandName.FullData);
			}
			else {
				ispResponse.setCommandType(ispData.getCommand_name());
			}
