...
@Component
public class EventDataCaster {
	
	private static final Logger LOG = LoggerFactory.getLogger(EventDataCaster.class);
	...
	public void prepareSinglePhase(String outputList, List<EventsSinglePhaseResponse> ispResponseList, MeterDataVisualizationReq req) throws Exception{
		...
		for (EventDataSinglePhase ispData : singlePhaseData) {
			try {
				...
				ispResponse.setEventCategory(ispData.getEvent_category() != null ? ispData.getEvent_category() : req.getReplaceBy());
				ispResponse.setEventCode(ispData.getEvent_code() != null ? String.valueOf(ispData.getEvent_code()) : req.getReplaceBy());
				ispResponse.setEventDatetime(ispData.getEvent_datetime() != null ? dateConverter.convertDateToHesString(ispData.getEvent_datetime()) : req.getReplaceBy());
