...
@Component
public class ConfigurationReadDataCaster {
	private static final Logger LOG = LoggerFactory.getLogger(ConfigurationReadDataCaster.class);
...
//			ispResponse.setUserId(String.valueOf(ispData.getUserId()));
	...
	public void prepareConfigurationReadData(String outputList, List<ConfigurationReadDataResponse> ispResponseList, MeterDataVisualizationReq req)
		...
		for (PrepayData ispData : prepayData) {
			...
			ispResponse.setDeviceSerialNumber(ispData.getDevice_serial_number() != null ? String.valueOf(ispData.getDevice_serial_number()) : req.getReplaceBy());
			ispResponse.setMdasDateTime(ispData.getMdas_datetime() != null ?
			...
			if(ConfigCommands.BILLING_DATES.commandName.equalsIgnoreCase(req.getCommand())) {
				...
				ispResponse.setRelayStatus(ispData.getRelay_status() != null ? ispData.getRelay_status() : req.getReplaceBy());
			}
			else if(ConfigCommands.APN.commandName.equalsIgnoreCase(req.getCommand())) {
				ispResponse.setApn(ispData.getApn() != null ? ispData.getApn() : req.getReplaceBy());
			}
			else if(ConfigCommands.IPV6_ADDRESS.commandName.equalsIgnoreCase(req.getCommand())) {
				ispResponse.setIpv6Address(ispData.getIpv6_address() != null ? ispData.getIpv6_address() : req.getReplaceBy());
			}
			else{
				ispResponse.setBillingDate(ispData.getBilling_dates() != null ?
						ispData.getBilling_dates() : req.getReplaceBy());
	...
	public void prepareInvalidRelayStatus(String outputList, List<ConfigurationReadDataResponse> ispResponseList, MeterDataVisualizationReq req)
			throws Exception {
		LOG.info("Configuration Read Data Caster called....");
		List<PrepayData> prepayData = new ArrayList<PrepayData>();
		prepayData = CommonUtils.getMapper().readValue(outputList, new TypeReference<List<PrepayData>>() {
		...
		for (PrepayData ispData : prepayData) {
			...
			ispResponse.setMdasDateTime(ispData.getMdas_datetime() != null ?
					dateConverter.convertDateToHesString(ispData.getMdas_datetime()) : req.getReplaceBy());
			ispResponse.setRelayStatus(ispData.getRelay_status() != null ? ispData.getRelay_status() : req.getReplaceBy());
			ispResponse.setCurrentRelayStatus(ispData.getCurrent_relay_status() != null ? ispData.getCurrent_relay_status() : req.getReplaceBy());
	...
	public void prepareRecentConfigurationReadData(String outputList, List<ConfigurationReadDataResponse> ispResponseList, MeterDataVisualizationReq req)
			throws Exception {
		LOG.info("Recent Configuration Read Data Caster called....");
		List<DevicesInfo> devInfo = new ArrayList<DevicesInfo>();
		devInfo = CommonUtils.getLogMapper().readValue(
				outputList, new TypeReference<List<DevicesInfo>>() {});

		LOG.info("Recent Configuration Read Response is preparing..");
		for (DevicesInfo ispData : devInfo) {
			ConfigurationReadDataResponse ispResponse = new ConfigurationReadDataResponse();

			ispResponse.setDeviceSerialNumber(String.valueOf(ispData.getDevice_serial_number()));
			ispResponse.setProfileCapturePeriod(String.valueOf(ispData.getProfile_capture_period()));
			ispResponse.setFirmwareVersion(String.valueOf(ispData.getVersion()));
			ispResponse.setDevMode(String.valueOf(ispData.getDev_mode()));
			ispResponse.setBillingDates(ispData.getBilling_datetime() != null ?
					ispData.getBilling_datetime() : req.getReplaceBy());
