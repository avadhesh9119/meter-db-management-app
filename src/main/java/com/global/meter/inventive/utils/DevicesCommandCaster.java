...
@Component
public class DevicesCommandCaster {

	private static final Logger LOG = LoggerFactory.getLogger(DevicesCommandCaster.class);
	...
	public void prepareDeviceCommands(String outputList, List<DeviceCommandResponse> ispResponseList, MeterDataVisualizationReq req) throws Exception {
		...
		for (DevicesCommand ispData : deviceCommandData) {
			...
			ispResponse.setCommandName(ispData.getCommand_name() != null ? String.valueOf(ispData.getCommand_name()) : req.getReplaceBy());
			ispResponse.setCommandCompletionDatetime(ispData.getCommand_completion_datetime() != null
					? dateConverter.convertDateToHesString(ispData.getCommand_completion_datetime())
					: req.getReplaceBy());
			ispResponse.setStatus(ispData.getStatus() != null ? String.valueOf(ispData.getStatus()) : req.getReplaceBy());
