...
@Component
public class DevicesConfigBatchLogsCaster {
	private static final Logger LOG = LoggerFactory.getLogger(DevicesConfigBatchLogsCaster.class);
	...
	batchLogs.setSource(ispData.getSource());
	batchLogs.setUser_id(ispData.getUser_id());
	owners.add(ispData.getOwner() != null ? ispData.getOwner() : "-");
	subdevisions.add(ispData.getSubdevision() != null ? ispData.getSubdevision() : "-");
	substations.add(ispData.getSubstation() != null ? ispData.getSubstation() : "-");
	feeders.add(ispData.getFeeder() != null ? ispData.getFeeder() : "-");
	dts.add(ispData.getDt() != null ? ispData.getDt() : "-");
	meters.add(ispData.getMeterNo() !=null ? ispData.getMeterNo() : "-");
  }
	...
	public void prepareDevicesConfigBatchLogs(String outputList,
		...
		for (DevicesConfigBatchLogs ispData : batchRequestList) {
			...
			ispResponse.setHierName(ExternalConstants.getUIBatchLevelName(ispData.getHier_name()));
	...
	public void prepareDevicesConfigReadBatchLogs(String outputList,
		...
		for (DevicesConfigReadBatchLogs ispData : batchRequestList) {
			...
			ispResponse.setHierValue(ispData.getHier_value());
			ispResponse.setHesDatetime(dateConverter.convertDateToHesString(ispData.getMdas_datetime()));
			ispResponse.setSuccess(String.valueOf(ispData.getSuccess()));
			ispResponse.setFailure(String.valueOf(ispData.getFailure()));
			ispResponse.setInProgress(String.valueOf(ispData.getIn_progress()));
			ispResponse.setCommandName(ispData.getCommand_name());
			ispResponse.setSource(ispData.getSource());
			ispResponse.setUserId(ispData.getUser_id());
