...
@RestController
@RequestMapping(value = "/hes/subdivisions")
public class SubdivisionsMasterController {

	private static final Logger LOG = LoggerFactory.getLogger(SubdivisionsController.class);

	@Autowired
	SubdivisionsMasterService subdivisionsService;
	...
	public ResponseEntity<Object> addSubdivision(
			@Valid @RequestBody List<SubdivisionsMasterDataRequest> subdivisionsDataRequests) {
...
//	public ResponseEntity<Object> deleteSubdivisions(
//			@Valid @RequestBody SubdivisionsMasterDataRequest subdivisionsDataRequests) {
//		LOG.info("Request recieved to delete subdivision..." + subdivisionsDataRequests.getHesSubdivisionId());
//		CommonResponse response = new CommonResponse();
//		try {
//
...
//	}
	...
	public ResponseEntity<Object> getSubdivisions(
			@Valid @RequestBody SubdivisionsMasterDataRequest subDivisionsDataRequests) {
		LOG.info("Request recieved to get subdevision informations...");
		CommonResponse response = new CommonResponse();
		try {
	...
	public ResponseEntity<Object> getSubdivisionList(
			@Valid @RequestBody SubdivisionsMasterDataRequest subDivisionDataRequest) {
		LOG.info("Request recieved to get subdivision list...");
		CommonResponse response = new CommonResponse();
		try {
	...
	@RequestMapping(value = "/get-list-by-division", method = RequestMethod.POST, consumes = "application/json", produces = "application/json")
	public ResponseEntity<Object> getSubdivisionNameListByDivision(
			@Valid @RequestBody SubdivisionsMasterDataRequest subDivisionDataRequest) {
		LOG.info("Request recieved to get subdivision list by division..." + subDivisionDataRequest.getDivisionCode());
		CommonResponse response = new CommonResponse();
		try {

			response = subdivisionsService.getSubdivisionNameListByDivision(subDivisionDataRequest);
			
			LOG.info("Request success to get list of subdivisions informations by division");

			return new ResponseEntity<>(response, HttpStatus.OK);

		} catch (Exception e) {
			LOG.error("Issue in getting list of subdivisions due to:" + e.getMessage());
			response = ExceptionHandlerConfig.setErrorData(e);
			return new ResponseEntity<>(response, HttpStatus.NOT_FOUND);
		}
	}
...
//			@Valid @RequestBody SubdivisionsMasterDataRequest subDivisionDataRequest) {
//		LOG.info("Request recieved to get subdivision list by utility..." + subDivisionDataRequest.getOwnerId());
//		CommonResponse response = new CommonResponse();
//		try {
//
